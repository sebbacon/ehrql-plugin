...
- Select the ehrQL command using the OpenSAFELY-first probe in `SKILL.md`.
- Preferred command for dummy-data smoke tests:
...
- Preferred assurance command:
  `opensafely exec ehrql:v1 assure analysis/test_dataset_definition.py`
- If the OpenSAFELY probe fails, replace the command prefix with `.venv/bin/ehrql` or `uv run ehrql` as described in `SKILL.md`.
